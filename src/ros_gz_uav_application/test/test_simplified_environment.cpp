...
int main()
{
    std::cout << "Testing Simplified Octomap-based Environment Implementation" << std::endl;
    ...
    // Test cost
    unsigned char cost = env.GetMapCost(voxel_x, voxel_y, voxel_z);
    std::cout << "Position (" << voxel_x << ", " << voxel_y << ", " << voxel_z 
              << ") cost: " << (int)cost << std::endl;
    
    // Test bounds checking
    bool within_bounds = env.IsCellWithinMap(voxel_x, voxel_y, voxel_z);
    std::cout << "Position (" << voxel_x << ", " << voxel_y << ", " << voxel_z 
              << ") within bounds: " << (within_bounds ? "Yes" : "No") << std::endl;
    ...
    env.GetEnvBounds(min_x, min_y, min_z, max_x, max_y, max_z);
    std::cout << "Environment bounds: (" << min_x << ", " << min_y << ", " << min_z 
              << ") to (" << max_x << ", " << max_y << ", " << max_z << ")" << std::endl;
    
    // Test resolution
    double res_xy = env.GetResolutionXY();
    double res_z = env.GetResolutionZ();
    std::cout << "Resolution XY: " << res_xy << ", Z: " << res_z << std::endl;
    
    std::cout << "All tests passed!" << std::endl;
    std::cout << "Memory usage is now minimal and scales with actual octomap occupancy." << std::endl;
